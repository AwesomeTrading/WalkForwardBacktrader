...
class BlackBox:
    ...
    @ray.remote
    def cubetobox_r(*args, **kwargs):
        cubetobox(*args, **kwargs)

    # go from normalized values (unit cube) to absolute values (box)
    def cubetobox(f, points, box, d, fmax=None):
        r = []
        c = []
    ...
    def initial_sampling(f, points, batch, n, box, d):
        result_ids = []
        for i in range(n // batch):
            result_ids.append(
                cubetobox_r.remote(
                    f=f, points=points[batch * i : batch * (i + 1), 0:-1], box=box, d=d,
                )
            )
    ...
    def refine_scaling_matrix(
        points, batch, T, d, nrand, nrand_frac, rho0, m, n, p, fmax
    ):
