...
import blackbox_multi3 as bb

# from blackbox_multi3 import BlackBox
import math
...
def getData(start_timestamp, end_timestamp):
    ...
    frame = frame[
        (frame["time"] >= pd.to_datetime(start_timestamp, unit="s"))
...
def RunBackTest(
    ...
    cerebro.addstrategy(
        MyStrategy, interval_params=params["interval_params"], printlog=shouldPrint
    )
    ...
    try:
        total_pnl = (
            returns_anlaysis["end"] - returns_anlaysis["start"]
        )  # ending_portfolio_value - starting_portfolio_value # trade_anlaysis['pnl']['gross']['total']
        ...
        return {
            ...
            "trade_analysis": trade_anlaysis,
            "value_anlaysis": value_anlaysis,
        }
    except:
        if shouldPrint:
            print("Probably no trades were made")
        return {
            "win_ratio": 1.0,
            "percentage_pnl": 0.0,
            "sharperatio": 0.0,
...
params["interval_params"] = [
    (interval_params_one_time, interval_params_one),
...
limitedEntryTestResult = RunBackTest(
    coin_name, candle_freq, 200000.0, start_date_with_buffer, tend, params, True, True
)
...
def BlackBoxParallelWalkForwardAnalysis(
    ...
    while optimization_end_date < end_date:
        ...
        print(
            "Optimizing for :  ",
            optimization_start_date_key.split(" ")[0],
            " - ",
...
def opt_fun(paramsToOptimize):
    ...
    testResult = RunBackTest(
        coin_name, candle_freq, 200000.0, tstart, tend, params, False, False
    )
...
def getIntervalParamsFromOptimizedParams(
    optimized_params_list, out_of_sample_period, which_best
):
    ...
    for optimization_date_key in sorted_opt_params:
        ...
        optimization_end_date = time.mktime(
            time.strptime(optimization_end_date_key, "%Y-%m-%d %H:%M:%S")
        )
        ...
        if out_of_sample_start_date_with_buffer == 0:
            ...
            out_of_sample_start_date_with_buffer = (
                out_of_sample_start_date - needed_lookback_buffer_in_seconds
            )
    ...
    return [
        interval_params,
        out_of_sample_start_date_with_buffer,
        out_of_sample_start_date_to_log,
        end_date,
    ]


def generateAlternateOutOfSampleResultsWithInterval(
    optimized_params_list, out_of_sample_period, top_best, shouldPlot
):
    ...
    for i in range(top_best):
        ...
        ] = getIntervalParamsFromOptimizedParams(
            optimized_params_list, out_of_sample_period, i
        )
        ...
        out_of_sample_start_date_with_buffer_key = str(
            time.strftime(
                "%Y-%m-%d %H:%M:%S",
                time.localtime(out_of_sample_start_date_with_buffer),
            )
        )
        ...
        out_of_sample_key = (
            out_of_sample_start_date_with_buffer_key + "_" + out_of_sample_end_date_key
        )

        # Run the out of sample Period with the optimized params
        OutOfSampleResults = RunBackTest(
            coin_name,
            candle_freq,
            200000.0,
            out_of_sample_start_date_with_buffer,
        ...
        print(
            "Out of sample testing for start date "
...
WalkForwardAnalysisResultsPNLAll_LastYear = BlackBoxParallelWalkForwardAnalysis(
    tstart,
    tend,
    optimization_period,
    out_of_sample_period,
