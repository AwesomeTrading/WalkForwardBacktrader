...
class FakeNumpy:
    ...
    def __init__(self, filename):
        ...
        if os.path.isfile(self.filename + str(self.raypos)):
            self.load(self.raypos)
            self.first = False
        else:
            self.ray_arrays[self.filename + str(self.raypos)] = []
            self.ray_pos[self.filename + str(self.raypos)] = 0
            self.first = True
    ...
    def realrand(self, *args):
        ...
        self.ray_arrays[self.filename + str(self.raypos)].append(rand)
        return rand
    ...
    def random(self, *args):
        if self.first:
            ...
            self.ray_arrays[self.filename + str(self.raypos)][
                self.ray_pos[self.filename + str(self.raypos)]
            ]
            self.ray_pos[self.filename + str(self.raypos)] += 1
        return rand
    ...
    def save(self, raypos):
        if not os.path.isfile(self.filename + str(self.raypos)):
            outfile = open(self.filename, "wb")
            pickle.dump(self.ray_arrays[self.filename + str(self.raypos)], outfile)
            outfile.close()
            return True
        return False
    ...
    def load(self, raypos):
        if os.path.isfile(self.filename + str(raypos)):
            ...
            self.ray_arrays[self.filename + str(raypos)] = pickle.load(infile)
            infile.close()
        else:
            self.ray_fname = self.filename + str(self.raypos)
            self.ray_arrays[self.filename + str(self.raypos)] = []
            self.ray_pos[self.filename + str(self.raypos)] = 0
            self.first = True
...
def search(
    ...
    print("------------------- PICKLEFILE:", fakenp.ray_fname)
    print("------------------- RAY POSITION:", raypos)
    print("------------------- FAKENP RAY POSITION:", fakenp.raypos)

    print(fakenp.ray_pos)
    print(fakenp.ray_arrays)
    print(fakenp.ray_fname)
    # space size
    d = len(box)
    ...
    # print("------------------- POSITION:", fakenp.pos)

    fakenp.save(raypos)
    print(fakenp.ray_pos)
    print(fakenp.ray_arrays)
    print("---------------------------_", fakenp.raypos)
    return points
